...
async function notificarApoderadoSalida(datos) {
    try {
        // Leemos la variable como un String y la comparamos para tener un Booleano real
        const enviarAReales = process.env.ENVIAR_CORREOS_REALES === 'true';

        // Si es true, va al apoderado. Si es false, va al gmail de prueba definido en el .env
        const destinoFinal = enviarAReales
            ? datos.correo_apoderado
            : process.env.CORREO_TEST_DESARROLLO;

        // Diseño html/css del correo
        const plantillaHTML = `
    ...
    `;
        ...
        // Disparamos el correo usando Resend
        await resend.emails.send({
            from: 'Plataforma Asistencia <onboarding@resend.dev>', // Dominio sandbox gratuito de Resend
            to: destinoFinal,
